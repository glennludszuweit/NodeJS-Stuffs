import { ContactDataSourceInterface } from '../src/data/interfaces/contact-datasource-interface';
import {
  ContactRequestModel,
...
class MockContactDataSource implements ContactDataSourceInterface {
...
describe('Contact Repository', () => {
  let mockContactDataSource: ContactDataSourceInterface;
  let contactRepository: ContactRepositoryInterface;
  ...
  describe('createContact', () => {
    test('should make create ds call', async () => {
      ...
      expect(mockContactDataSource.create).toHaveBeenCalledWith({
        name: 'Smith',
      });
    });
  });
  ...
  describe('deleteContact', () => {
    test('should make deleteOne ds call', async () => {
      jest
        .spyOn(mockContactDataSource, 'deleteOne')
        .mockImplementation(() => Promise.resolve());
  ...
  describe('updateContact', () => {
    test('should make updateOne ds call', async () => {
      jest
        .spyOn(mockContactDataSource, 'updateOne')
        .mockImplementation(() => Promise.resolve());
      await contactRepository.updateContact('123', { name: 'John' });
      expect(mockContactDataSource.updateOne).toHaveBeenCalledWith('123', {
        name: 'John',
      });
    });
  });
  ...
  describe('getContact', () => {
    test('should make getOne ds call', async () => {
      jest
        .spyOn(mockContactDataSource, 'getOne')
        .mockImplementation(() => Promise.resolve({ id: '123', name: 'Paul' }));
