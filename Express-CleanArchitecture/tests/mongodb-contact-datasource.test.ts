import { NoSQLDatabaseWrapperInterface } from '../src/data/interfaces/nosql-database-wrapper-interface';
import { MongoDBContactDataSource } from '../src/data/mongodb/mongodb-contact-datasource';

describe('MongoDB DataSource', () => {
  let mockDatabase: NoSQLDatabaseWrapperInterface;
  ...
  test('getAll', async () => {
    ...
    jest
      ...
      .mockImplementation(() =>
        Promise.resolve([{ name: 'Smith', _id: '123' }])
      );
  ...
  test('getOne', async () => {
    ...
    jest
      .spyOn(mockDatabase, 'find')
      .mockImplementation(() =>
        Promise.resolve([{ _id: '123', name: 'Smith' }])
      );
